...
if(isset($_POST["id"]) && !empty($_POST["id"])){
	...
	if(empty($fileName)){
		...
		$link=($_SERVER['DOCUMENT_ROOT'] . "/shop/product_images/" . $product_image_hidden);
		if(file_exists($link))
		{unlink($link);}
    ...
    if(empty($name_err) ){
        ...
        if($stmt = mysqli_prepare($con, $sql)){
            ...
            mysqli_stmt_bind_param($stmt, "sssssssi", $param_cat_id ,$param_brand_id ,$param_product_title ,
									$param_product_price ,$param_product_desc ,$param_product_image, $param_product_keywords, $param_id);
										...
										if($result_cat = mysqli_query($con, $sql_cat)){										
											if(mysqli_num_rows($result_cat) > 0){
												while($row_cat = mysqli_fetch_array($result_cat)){
													$selected_cat = "";
													if ( $cat_id == $row_cat['cat_id'] ){$selected_cat = "selected";}
										...
										if($result_brand = mysqli_query($con, $sql_brand)){
											if(mysqli_num_rows($result_brand) > 0){
												while($row_brand = mysqli_fetch_array($result_brand)){													
													$selected = "";
													if ( $brand_id == $row_brand['brand_id'] ){$selected = "selected";}
